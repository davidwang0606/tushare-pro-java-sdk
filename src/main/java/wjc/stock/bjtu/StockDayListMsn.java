...
public class StockDayListMsn extends StockDayList implements Runnable {
	public static String MsnBase = "http://app.finance.ifeng.com/hq/stock_daily.php?code=";
	public static String DayDir = "DayExcel";
	public static String StartDay = "&begin_day=2015-06-21";
	
	
	
	public StockDayListMsn(StockInfo s) {
		super(s);
		// TODO Auto-generated constructor stub
	}
	
	public boolean downloadFromMsn(){
		daylist.clear();
		String code = stock.getCode();
		char first = code.charAt(0);
		String u;
		if(first=='6'){
			u = MsnBase+"sh"+stock.getCode()+StartDay;
	//		u = MsnBase+stock.getCode();
		} else if(first == '0' || first == '3'){
		    u = MsnBase+"sz"+stock.getCode()+StartDay;
	//	    u = MsnBase+stock.getCode();
		}else {
			u = MsnBase+stock.getCode()+StartDay;
	//		u = MsnBase+stock.getCode();
		}
...
//					System.out.println(line);
					...
					index = content.indexOf(((String)v.get(0)+"</td>"),index);
...
//		System.out.println("index:"+index);
	...
	public int updateToExcelWithLocal(){
		boolean b = true;
		int downloadsize = daylist.size()-1;
		int index = 0;
		int lines = 0;
...
//		System.out.println("index:"+index);
	...
	public int readFromFile(int length){
		if(length <=0){
			System.out.println("StockDayListMsn:readFromFile ="+length);
			return 0;
		}
		ExcelAccesser exc = new ExcelAccesser(DayDir+File.separator+stock.getCode()+".xls");
		...
		if(!openState){
			System.out.println("StockDayListMsn:readFromFile open File "+stock.getCode()+" fail");
			return 0;
		}
		int index = 0;
		while(index < length){
...
//		}
	...
	public StockDayListMsn getStockDayListWithNull(){
		return new StockDayListMsn(new StockInfo(this.getStock().getCode(),this.getStock().getName()));
	}
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
//		StockInfo s = new StockInfo("300156","天立环保");
//		StockDayListMsn list = new StockDayListMsn(s);
//		int n = 0;
//		n= list.readFromFile(90);
//		list.orderByClosePrice(2);
//		list.createAvgDown();
//		System.out.println(list);
//		System.out.println(n);
//		System.out.println(((DaytimeStock)(list.getDayList().getFirst())).getClosePrice());
//		list.downloadFromMsn();
		
//		list.saveToExcel();
//		int row = list.updateToExcel();
//		System.out.println(row);
//		list.RefreshData();
		StockInfo s = new StockInfo("000001","平安银行");
		StockDayListMsn list = new StockDayListMsn(s);
		int n = list.readFromFile(50);
		System.out.println(n);
